...
var SYSCALLS = {
 ...
 doStat: function(func, path, buf) {
  ...
  SAFE_HEAP_STORE((buf + 32 >> 2) * 4, 0, 4);
...
function readAsmConstArgs(sigPtr, buf) {
 ...
 while (ch = SAFE_HEAP_LOAD(sigPtr++, 1, 1)) {
  buf += ch != 105 & buf;
  readAsmConstArgsArray.push(ch == 105 ? SAFE_HEAP_LOAD(buf * 4, 4, 0) : SAFE_HEAP_LOAD_D((buf++ >> 1) * 8, 8, 0));
  ++buf;
 }
